...
pipeline {
    ...
    stage('SonarQube Inspection') {
        steps {
            withSonarQubeEnv('SonarQube') { 
                withCredentials([string(credentialsId: 'SonarGube-Token', variable: 'SONAR_TOKEN')]) {
                sh """
                mvn sonar:sonar \
                -Dsonar.projectKey=cicd-pipeline-project \
                -Dsonar.host.url=http://172.31.49.93:9000 \
  ...
  post {
    always {
        ...
        slackSend channel: '#eec-jenkins-cicd-pipeline-alerts', //update and provide your channel name
